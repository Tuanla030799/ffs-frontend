  ...
  >
    <div class="space-y-3">
      <div class="flex flex-wrap items-center gap-2">
        <UiBadge variant="primary" label="/guide" />
        <UiBadge variant="success" label="Reusable patterns" />
      </div>
      <div>
        <h1 class="text-2xl font-bold tracking-tight text-[var(--ui-text)] sm:text-3xl">
          Guide & pattern library
        </h1>
        <p class="mt-2 max-w-3xl text-sm leading-6 text-[var(--ui-text-muted)] sm:text-base">
          Toàn bộ phần hướng dẫn, demo component và pattern tích hợp Axios + Pinia được gom về đây
          để màn
    ...
    <div class="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
      <UiCard title="Route map" description="Sơ đồ route hiện tại để bạn mở rộng sau này.">
        <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
          <UiCard bordered :shadow="false" padding="sm" class="border-dashed">
            <div class="space-y-2">
              <UiBadge variant="primary" label="/fds" />
          ...
          <UiCard bordered :shadow="false" padding="sm" class="border-dashed">
            <div class="space-y-2">
              <UiBadge variant="success" label="/guide" />
              <p class="text-sm text-[var(--ui-text-muted)]">
          ...
          <UiCard bordered :shadow="false" padding="sm" class="border-dashed">
            <div class="space-y-2">
              <UiBadge variant="warning" label="/admin" />
              <p class="text-sm text-[var(--ui-text-muted)]">
                Khu admin tách layout riêng, phù hợp CRUD, template manager, asset manager.
              </p>
            </div>
          </UiCard>
        </div>
      ...
      <UiCard title="Quick links" padding="md">
        <div class="space-y-3">
          <RouterLink v-for="item in links" :key="item.to" :to="item.to" class="block no-underline">
            <div
    ...
    <div class="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
      <UiCard title="Mini design system" padding="md">
        <div class="space-y-4">
          <UiForm>
            <UiFormItem
            ...
            >
              <UiRadio v-model="sampleTemplate" :options="TEMPLATE_OPTIONS" stacked />
            </UiFormItem>
          ...
          <div class="flex flex-wrap gap-2">
            ...
            <UiTag label="Template active" variant="primary" />
            <UiTag label="Closable tag" closable @close="onTagClose" />
          </div>

          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <UiDropdown :items="MENU_ITEMS" trigger-text="Menu hành động" @select="onMenuSelect" />
            <UiPopover
              title="Mẹo mobile"
          ...
          <div class="grid grid-cols-2 gap-3">
            <UiButton block variant="secondary" @click="openDrawer = true">
              Mở drawer demo
            </UiButton>
            <UiButton
              block
              variant="outline"
              @click="
                pushToast('Đây là toast mẫu để bạn gắn vào action thành công.', 'success', 'Saved')
              "
            >
              Bắn toast
            </UiButton>
      ...
      >
        <UiForm class="space-y-4">
          ...
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <UiButton :loading="apiLoading" block @click="fetchTemplateList">
              Gọi GET /templates
            </UiButton>
            <UiButton
              block
    ...
    <div class="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
      ...
      >
        ...
        >
          ...
          <template #cell-status="{ value }">
            <UiBadge :label="value as any" :variant="badgeVariant(value as any)" />
          </template>
          <template #cell-tags="{ value }">
            <div class="flex min-w-[160px] flex-wrap gap-2">
              <UiTag v-for="tag in value" :key="tag" :label="tag as any" variant="outline" />
            </div>
          </template>
          <template #cell-actions="{ row }">
            <div class="flex min-w-[160px] gap-2">
              <UiButton size="sm" variant="ghost" @click="previewTemplate(row)"> Preview </UiButton>
              <UiButton size="sm" variant="outline" @click="duplicateTemplate(row)">
                Clone
              </UiButton>
            </div>
          </template>
      ...
      <div class="space-y-4">
        <UiCard title="Empty state + loading" padding="md">
          <div class="space-y-4">
            <UiEmpty
            ...
            >
              <template #action>
                <UiButton
                  size="sm"
                  @click="pushToast('Tạo template mới từ empty state.', 'info', 'Create')"
                >
                  Tạo template mới
                </UiButton>
              </template>
            ...
            <UiCard bordered :shadow="false" padding="sm" class="border-dashed">
              <div class="space-y-3">
                <UiSkeleton avatar :rows="3" />
                <UiSkeleton :rows="2" compact />
              </div>
            </UiCard>
          </div>
        </UiCard>

        <UiCard title="Badge trạng thái" padding="md">
          <div class="flex flex-wrap gap-2">
            <UiBadge label="active" variant="success" />
            <UiBadge label="draft" variant="warning" />
            <UiBadge label="archived" variant="neutral" />
            <UiBadge label="error" variant="danger" />
          </div>
        </UiCard>
      </div>
    </div>
    ...
    >
      <UiForm class="space-y-4">
        ...
        <UiButton block @click="saveDrawerTemplate"> Lưu template demo </UiButton>
      </UiForm>
    </UiDrawer>

    <UiToast :items="toasts" @remove="removeToast" />
  </section>
</template>
