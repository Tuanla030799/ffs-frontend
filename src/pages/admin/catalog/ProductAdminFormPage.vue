<template>
  <section class="space-y-5">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <p class="text-sm font-bold uppercase text-slate-400">Products</p>
        <h2 class="text-2xl font-black text-slate-950">{{ isEdit ? 'Cập nhật product' : 'Tạo product' }}</h2>
      </div>
    ...
    <form class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]" @submit.prevent="save">
      <div class="space-y-5">
        <UiCard title="Thông tin cơ bản" padding="md">
          <div class="grid gap-3 md:grid-cols-3">
            ...
            <UiSelect v-model="form.categoryId" placeholder="Category *" label="Category">
              <option v-for="category in masterData?.categories || []" :key="category.id" :value="category.id">{{ category.name }}</option>
            </UiSelect>
            ...
            <UiInput v-model="form.shortDescription" class="md:col-span-2" placeholder="Short description"  label="Short description"/>
            <UiSelect v-model="form.status" label="Trạng thái">
              <option v-for="status in masterData?.productStatuses || []" :key="status.value" :value="status.value">{{ status.label }}</option>
            </UiSelect>
            <UiCheckbox v-model="form.isFeatured" label="Featured" />
            <UiInput v-model.number="form.featuredOrder" placeholder="Featured order"  label="Featured order"/>
          </div>
        </UiCard>
        ...
        <UiCard title="Variants" padding="md">
          <template #extra>
            <UiButton native-type="button" variant="outline" size="sm" @click="addVariant">+ Variant</UiButton>
          </template>
          <div class="grid gap-3 md:grid-cols-2">
            <div v-for="variant in variantList" :key="variant.clientId || variant.id" class="grid gap-2 rounded-xl border border-slate-200 p-3">
              ...
              <UiInput v-model="variant.imageFileId" placeholder="Image file ID"  label="Image file ID"/>
        ...
        <UiCard title="SKUs" padding="md">
          <template #extra>
            <UiButton native-type="button" variant="outline" size="sm" @click="addSku">+ SKU</UiButton>
          </template>
          <div class="grid gap-3 md:grid-cols-2">
            <div v-for="sku in skuList" :key="sku.id || sku.skuCode" class="grid gap-2 rounded-xl border border-slate-200 p-3">
              <UiInput v-model="sku.variantClientId" placeholder="variantClientId"  label="Variant client ID"/>
      ...
      <aside class="space-y-5">
        <UiCard title="Images" padding="md">
          ...
          <div class="mt-3">
            <UiButton native-type="button" variant="outline" size="sm" @click="addEmptyImage">+ Image row</UiButton>
          </div>
        ...
        <UiCard title="Actions" padding="md">
          <p class="mt-1 text-sm text-slate-500">Form này tách route riêng để dễ nhập dữ liệu dài.</p>
...
const form = reactive<ProductPayload>({ categoryId: '', brandId: null, gender: 'UNISEX', name: '', slug: '', shortDescription: '', descriptionJson: { time: Date.now(), blocks: [] }, status: 'ACTIVE', isFeatured: false, featuredOrder: 0, images: [], variants: [], skus: [] })
...
function goBack() { void router.push({ name: 'admin-products' }) }

async function save() {
  const msg = required(form.name, 'Name') || required(form.slug, 'Slug') || required(form.categoryId, 'Category ID')
