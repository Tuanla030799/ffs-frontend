<template>
  <main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 md:py-10 lg:px-8">
    ...
    <div v-else-if="collection" class="space-y-12">
      <section class="grid overflow-hidden bg-[#f7f7f5] shadow-sm ring-1 ring-black/5 lg:grid-cols-[1.1fr_0.9fr]">
        <img
        ...
        <div class="flex flex-col justify-center p-5 md:p-12">
          <p class="text-sm font-bold uppercase tracking-[0.18em] text-black/50">Collection</p>
          <h1 class="mt-4 text-3xl font-black uppercase leading-none md:text-5xl">{{ collection.name }}</h1>
...
const products = computed<Product[]>(() => collection.value?.products?.map((item) => item.product).filter(Boolean) as Product[] || [])
