  ...
  <main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 md:py-12 lg:px-8">
    <header class="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between md:mb-10">
      ...
      <FilterForm
        ...
        :card="false"
        class="flex gap-2"
        search-placeholder="Tìm brand"
        search-label="Tìm brand"
        submit-label="Tìm"
        @search="search"
      />
    </header>
      ...
      >
        <div class="grid aspect-[4/3] place-items-center bg-white">
          <img
            ...
            :src="imageUrl(row)"
            :alt="row.name"
            class="h-full w-full object-contain p-5 grayscale transition group-hover:grayscale-0"
          />
...
async function load() {
  loading.value = true
  try {
    rows.value = await brandApi.list({ keyword: keyword.value })
  } finally {
    loading.value = false
  }
}
function search() {
  void router.replace({ query: { keyword: keyword.value || undefined } })
  void load()
}
