<template>
  <main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 md:py-12 lg:px-8">
    <header class="mb-7 md:mb-10">
      <p class="text-sm font-bold uppercase tracking-[0.18em] text-black/50">Shop</p>
    ...
    <form class="mb-7 grid gap-3 border border-black/10 bg-[#f7f7f5] p-3 md:mb-8 md:grid-cols-6 md:p-4" @submit.prevent="search">
      <UiInput v-model="query.keyword" class="md:col-span-2" placeholder="Tìm giày"  label="Tìm giày"/>
      <UiSelect v-model="query.categorySlug" label="Danh mục">
        <option value="">Tất cả category</option>
        <option v-for="c in masterData?.categories || []" :key="c.id" :value="c.slug">{{ c.name }}</option>
      </UiSelect>
      <UiSelect v-model="selectedBrandSlug" label="Thương hiệu">
        <option value="">Tất cả brand</option>
        <option v-for="brand in masterData?.brands || []" :key="brand.id" :value="brand.slug">{{ brand.name }}</option>
      </UiSelect>
    ...
    <div v-if="error" class="mb-4 border border-red-200 bg-red-50 p-4 text-red-700">{{ error }}</div>
    ...
    <div v-else class="border border-dashed border-black/20 bg-[#f7f7f5] p-10 text-center text-black/60">Không tìm thấy sản phẩm.</div>
    ...
    <div class="mt-8 flex justify-center gap-2">
      ...
      <UiButton variant="outline" :disabled="page >= totalPages" @click="go(page + 1)">Sau</UiButton>
    </div>
  </main>
</template>
...
const query = reactive<ProductListQuery>({ keyword: String(route.query.keyword || ''), categorySlug: String(route.query.categorySlug || ''), brandSlug: selectedBrandSlug.value, gender: String(route.query.gender || ''), size: '', color: '', page: Number(route.query.page || 1), limit: 12 })
...
async function load() {
  ...
  try {
    const data = await productApi.list({ ...query, brandSlug: selectedBrandSlug.value || undefined, gender: query.gender || undefined })
...
function search() { query.page = 1; void router.replace({ query: routeQuery() }); void load() }
function go(next: number) { query.page = next; void router.replace({ query: routeQuery() }); void load() }

onMounted(async () => { await loadMasterData().catch(() => null); await load() })
</script>
