...
export const collectionApi = {
  async list(params?: CollectionListQuery) {
    ...
    return unwrapList<Collection>(res.data.data, params?.page, params?.limit) as Paginated<Collection>
  },
  async detail(slug: string, params?: CollectionListQuery) {
    const res = await httpClient.get<ApiEnvelope<Collection>>(`/api/collections/${slug}`, { params })
  ...
  async update(id: string, payload: CollectionPayload) {
    const res = await httpClient.put<ApiEnvelope<Collection>>(`/api/admin/collections/${id}`, payload)
    return res.data.data
  },
