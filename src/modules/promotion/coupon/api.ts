...
export const couponApi = {
  ...
  async adminList(params?: ListQuery) {
    const res = await httpClient.get<ApiEnvelope<unknown>>('/api/admin/coupons', { params })
    return unwrapList<Coupon>(res.data.data, params?.page, params?.limit) as Paginated<Coupon>
  },
  async create(payload: CouponPayload) {
    const res = await httpClient.post<ApiEnvelope<Coupon>>('/api/admin/coupons', payload)
    return res.data.data
  },
  async update(id: string, payload: CouponPayload) {
    const res = await httpClient.put<ApiEnvelope<Coupon>>(`/api/admin/coupons/${id}`, payload)
    return res.data.data
  },
