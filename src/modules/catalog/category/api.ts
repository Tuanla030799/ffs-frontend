...
export const categoryApi = {
  async publicList() {
    const res = await httpClient.get<ApiEnvelope<Category[]>>('/api/categories')
    return res.data.data || []
  },
  async adminList(params?: ListQuery) {
    const res = await httpClient.get<ApiEnvelope<unknown>>('/api/admin/categories', { params })
  ...
  async create(payload: CategoryPayload) {
    const res = await httpClient.post<ApiEnvelope<Category>>('/api/admin/categories', payload)
    return res.data.data
  },
  async update(id: string, payload: CategoryPayload) {
    const res = await httpClient.put<ApiEnvelope<Category>>(`/api/admin/categories/${id}`, payload)
    return res.data.data
  },
