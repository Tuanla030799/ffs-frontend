...
const editorInit = computed(() => ({
  ...
  toolbar:
    'undo redo | blocks | bold italic underline strikethrough | forecolor backcolor | alignleft aligncenter alignright alignjustify | bullist numlist | link image table blockquote | code preview fullscreen',
  ...
  content_css: false,
  content_style: `
    body { font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; font-size: 15px; line-height: 1.75; color: #111827; }
    img { max-width: 100%; height: auto; border-radius: 14px; }
    figure { margin: 1rem 0; }
    figcaption { color: #64748b; font-size: 13px; text-align: center; }
    table { width: 100%; border-collapse: collapse; }
    th, td { border: 1px solid #e2e8f0; padding: 10px; }
    blockquote { border-left: 4px solid #111827; margin-left: 0; padding-left: 16px; font-weight: 700; }
  `,
  images_upload_handler: async (blobInfo: BlobInfo) => {
    localError.value = ''
    try {
      const file = new File([blobInfo.blob()], blobInfo.filename(), {
        type: blobInfo.blob().type || 'image/*',
      })
      const uploaded = await fileService.upload(file, { scope: 'admin', fieldName: 'file' })
      if (!uploaded.url) throw new Error('Upload image failed')
      // TODO: backend should mark uploaded rich text image files as ACTIVE by parsing img src/path when content is saved.
      return uploaded.url
    } catch (error) {
