<template>
  <label :class="wrapperClasses">
    ...
    <span class="min-w-0 flex-1">
      <span v-if="label || $slots.default" class="block text-sm font-medium text-[var(--ui-text)]">
        <slot>{{ label }}</slot>
      </span>
      <span v-if="description" class="mt-1 block text-xs leading-5 text-[var(--ui-text-soft)]">{{
        description
      }}</span>
    </span>
  </label>
