<template>
  <div :class="wrapperClasses">
    <div>
      <div v-if="title" class="text-base font-semibold text-[var(--ui-text)]">
        {{ title }}
      </div>
      <div v-if="description" class="mt-1 text-sm text-[var(--ui-text-soft)]">
        {{ description }}
      </div>
    </div>
...
const wrapperClasses = computed(() =>
  cn(
    ...
    props.dashed
      ? 'border border-dashed border-[var(--ui-border-strong)]'
...
const triggerClasses = computed(() =>
  cn(
    ...
    props.disabled
      ? 'cursor-not-allowed bg-[var(--ui-surface)] text-[var(--ui-text-soft)]'
      : 'bg-[var(--ui-text)] text-white hover:opacity-90',
  ),
)
